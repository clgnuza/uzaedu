...
# Optional: DOMAIN_API, DOMAIN_ADMIN (default: api.uzaedu.com, admin.uzaedu.com) -> web-admin/.env.production
...
(
  cd web-admin
  printf '%s\n' "NEXT_PUBLIC_API_BASE_URL=https://${DOMAIN_API}/api" "NEXT_PUBLIC_SITE_URL=https://${DOMAIN_ADMIN}" > .env.production
...
pm2 save
echo "[deploy] OK $(date -Iseconds) branch=$BRANCH sha=$GIT_SHA_SHORT root=$ROOT api=https://${DOMAIN_API} admin=https://${DOMAIN_ADMIN}"