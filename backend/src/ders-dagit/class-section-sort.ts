...
export function sortValidationIssues<T extends { code: string; message: string }>(issues: T[]): T[] {
  const sectionCodes = new Set(['SECTION_NO_HOURS']);
  return [...issues].sort((a, b) => {
    const sa = sectionCodes.has(a.code) ? a.message.split(':')[0]?.trim() ?? '' : '';
    const sb = sectionCodes.has(b.code) ? b.message.split(':')[0]?.trim() ?? '' : '';
