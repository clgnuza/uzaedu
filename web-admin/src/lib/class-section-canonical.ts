import { CLASSROOM_DISPLAY_SUFFIX } from '@/lib/classroom-display-name';
import { compareClassSections, parseGradeFromClassSection, sortClassSections } from '@/lib/class-section-sort';

/** MTAL / alan adı gibi uzun okul kaydı şube adı */
...
/** Aynı sınıf+şube için tek anahtar (9-A ≈ AMP - 9/A) */
...
/** Aynı şubenin kısa/uzun yazılışlarını tek satırda göster (okul kaydı / uzun ad tercih) */
...
/** Tüm yazılış → tek resmi şube adı */
...
export function countSectionAliasGroups(sections: string[]): number {
  const keys = new Set(sections.map(sectionIdentityKey));
  return keys.size;
}

export function hadSectionAliases(sections: string[]): boolean {
  return sections.length > countSectionAliasGroups(sections);
}

export function sectionsMatch(a: string, b: string): boolean {
...
/** Derslik kaydı bu şubeye ait mi (ad veya allowed_class_sections) */
export function roomCoversSection(
  ...
  if (fromName && sectionsMatch(fromName, section)) return true;
  if (sectionsMatch(roomName, section)) return true;
  for (const a of allowed ?? []) {
    if (sectionsMatch(a, section)) return true;
  }
  return false;
}

/** Liste öğelerini resmi (uzun) şube adına çevirir; birleşik atamalarda sırayı korur */
export function canonicalizeSectionList(sections: string[]): string[] {
  return normalizeClassSectionNamesFromPool(
    sections.map((s) => s.trim()).filter(Boolean),
    sections,
  );
}

/** Havuzdaki tüm yazılışlarla eşleştirip tek resmi şube adı döner */
...
export const assignmentEffectiveWeeklyHours = effectiveAssignmentWeeklyHours;

/** Atama satırlarından şube haftalık saat (katalog yok; aynı ders+şube tekrarı tek). */
export function buildWeeklyHoursFromAssignments(
  ...
  for (const a of assignments) {
    ...
    if (hrs <= 0) continue;
    const subj = String(a.subject_name ?? '')
      .trim()
...
export function mergeRecordBySectionAlias(record: Record<string, number>): Record<string, number> {
