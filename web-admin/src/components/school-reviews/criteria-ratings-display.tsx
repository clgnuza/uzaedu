  ...
  if (isPublic) {
    return (
      <div className={cn('w-full min-w-0 max-w-full', className)}>
        ...
        >
          <ul
            className="flex w-max max-w-none snap-x snap-mandatory flex-nowrap gap-1 pb-0.5"
            aria-label="Kriter puanları"
          >
          {headerRating != null && (
            <li className="flex w-[4.25rem] shrink-0 snap-start flex-col justify-between rounded border border-amber-200/60 bg-amber-50/50 px-1.5 py-1 dark:border-amber-900/40 dark:bg-amber-950/25">
              <span className="text-[8px] font-medium uppercase leading-tight text-amber-800/90 dark:text-amber-200/90">
                Genel
              </span>
              <div className="mt-px flex items-baseline gap-px tabular-nums">
                <span className="text-xs font-bold leading-none text-foreground">
                  {headerRating.value.toFixed(1)}
                </span>
                <span className="text-[8px] text-muted-foreground">/{headerRating.max}</span>
              </div>
            </li>
          )}
          {entries.map(({ c, v }) => (
            <li
              key={c.slug}
              className="flex w-[4.25rem] shrink-0 snap-start flex-col justify-between rounded border border-border/70 bg-background/80 px-1.5 py-1"
            >
              <span
                className="line-clamp-2 text-[8px] font-medium leading-tight text-muted-foreground"
                title={c.label}
              >
                {c.label}
              </span>
              <div className="mt-0.5 text-center">
                <span className="inline-block rounded bg-emerald-500/12 px-1 py-px text-[9px] font-bold tabular-nums text-emerald-800 dark:text-emerald-200">
                  {v}/{c.max_score}
                </span>
              </div>
              <div
                className="mt-0.5 h-px w-full overflow-hidden rounded-full bg-muted"
                role="presentation"
              >
                <div
