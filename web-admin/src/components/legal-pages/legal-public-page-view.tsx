...
export function LegalPublicPageView({
  ...
  return (
    <div className="min-h-dvh w-full overflow-x-hidden">
      ...
      {/* ── Hero ─ koyu lacivert ── */}
      <div
        className="relative overflow-hidden pb-8 pt-4 sm:pb-11 sm:pt-6"
      ...
      >
        ...
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <div className="flex flex-col items-center text-center">
            <div className="mb-2 flex size-11 items-center justify-center rounded-xl border border-white/15 bg-white/10 shadow-lg backdrop-blur-sm sm:mb-2.5 sm:size-[52px]">
              <PageIcon className={cn('size-[22px] sm:size-6', meta.color)} />
            </div>
            <span className={cn(
              'mb-1.5 inline-flex items-center rounded-full border px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-widest',
              meta.badge,
            )}>
            ...
            <p className="mt-1.5 text-[11px] text-white/45">Son güncelleme: {updated}</p>
          </div>

          {/* Güven rozetleri */}
          <div className="mt-3 grid grid-cols-4 gap-1.5 sm:mt-3.5 sm:gap-2">
            {TRUST_ITEMS.map((t) => (
              <div
                key={t.label}
                className="flex flex-col items-center gap-0.5 rounded-xl border border-white/10 bg-white/6 px-1.5 py-1 text-center backdrop-blur-sm sm:flex-row sm:items-center sm:gap-1.5 sm:px-2 sm:py-1 sm:text-left"
              >
                <span className="shrink-0 text-[15px] leading-none sm:text-base">{t.emoji}</span>
                <div className="min-w-0 hidden w-full flex-col sm:flex">
                  <p className="text-[10px] font-bold leading-[1.05] text-white/85">{t.label}</p>
                  <p className="-mt-[3px] text-[8px] leading-[1.05] text-white/40 sm:-mt-1 sm:text-[9px]">{t.sub}</p>
                </div>
                <div className="flex w-full flex-col sm:hidden">
                  <p className="text-[8px] font-semibold leading-[1.05] text-white/75">{t.label}</p>
                  <p className="-mt-[3px] text-[7px] leading-[1.05] text-white/40">{t.sub}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="relative mx-auto -mt-6 max-w-3xl px-3 pb-6 sm:-mt-8 sm:px-6 sm:pb-10">
        <article className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-xl dark:border-slate-700/60 dark:bg-slate-950">
          ...
          {/* Üst şerit */}
          <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-4 py-2 dark:border-slate-800 dark:bg-slate-900 sm:px-5 sm:py-2.5">
          ...
          <div
            className={cn(
              'legal-html-content not-prose px-4 py-4 sm:px-6 sm:py-5 md:px-8',
              'text-[13px] leading-relaxed text-slate-600 dark:text-slate-400 sm:text-[15px] sm:leading-[1.65]',
              '[&_h2]:mt-4 [&_h2]:scroll-mt-24 [&_h2]:border-b [&_h2]:border-slate-100 [&_h2]:pb-1 [&_h2]:text-sm [&_h2]:font-bold [&_h2]:text-slate-800 dark:[&_h2]:border-slate-800 dark:[&_h2]:text-slate-100',
              'sm:[&_h2]:mt-6 sm:[&_h2]:text-base md:[&_h2]:text-lg',
              '[&_h2:first-child]:mt-0',
              '[&_h3]:mt-4 [&_h3]:text-sm [&_h3]:font-semibold [&_h3]:text-slate-700 dark:[&_h3]:text-slate-200 sm:[&_h3]:text-base sm:[&_h3]:mt-5',
              '[&_p]:mt-2.5 [&_p]:text-pretty [&_p]:first:mt-0 sm:[&_p]:mt-3',
              '[&_strong]:font-semibold [&_strong]:text-slate-800 dark:[&_strong]:text-slate-100',
              '[&_ul]:my-3 [&_ul]:list-disc [&_ul]:space-y-1 [&_ul]:pl-5 sm:[&_ul]:my-3.5 sm:[&_ul]:space-y-1.5',
              '[&_ol]:my-3 [&_ol]:list-decimal [&_ol]:space-y-1 [&_ol]:pl-5 sm:[&_ol]:my-3.5 sm:[&_ol]:space-y-1.5',
              '[&_li]:text-pretty',
              '[&_a]:text-blue-600 [&_a]:underline [&_a]:underline-offset-2 [&_a]:transition hover:[&_a]:text-blue-500 dark:[&_a]:text-blue-400',
              '[&_blockquote]:my-3 [&_blockquote]:border-l-4 [&_blockquote]:border-blue-400/40 [&_blockquote]:pl-4 [&_blockquote]:italic [&_blockquote]:text-slate-500',
            )}
            dangerouslySetInnerHTML={{ __html: block.body_html }}
          />

          {/* Alt navigasyon */}
          <div className="border-t border-slate-100 bg-slate-50/80 px-4 py-2.5 dark:border-slate-800 dark:bg-slate-900/60 sm:px-5 sm:py-3">
            <p className="mb-1.5 text-[9px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
              Diğer Politikalar & Tercihler
            </p>
            <nav className="flex flex-wrap gap-1.5 sm:gap-2" aria-label="Diğer yasal sayfalar">
              {links.map((l) => {
                ...
                return (
                  <Link
                    key={l.href}
                    href={l.href}
                    className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[12px] font-medium text-slate-600 shadow-sm transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-blue-500/40 dark:hover:bg-blue-900/20 dark:hover:text-blue-300 sm:px-3 sm:py-2"
                  >
              ...
              <CookiePreferencesLink className="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-[12px] font-medium text-amber-700 shadow-sm transition hover:bg-amber-100 dark:border-amber-700/40 dark:bg-amber-900/20 dark:text-amber-300 sm:px-3 sm:py-2">
                <Cookie className="size-3.5 shrink-0 opacity-70" />
                Çerez Tercihleri
              </CookiePreferencesLink>
              <Link
                href="/iletisim"
                className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-[12px] font-semibold text-blue-700 shadow-sm transition hover:bg-blue-100 dark:border-blue-700/40 dark:bg-blue-900/20 dark:text-blue-300 sm:px-3 sm:py-2"
              >
