...
export function Breadcrumb() {
  ...
  return (
    <nav
      ...
      className="mb-3 inline-flex max-w-full flex-wrap items-center gap-1 rounded-2xl border border-border/50 bg-muted/25 px-2.5 py-1.5 text-[13px] font-medium shadow-sm backdrop-blur-sm sm:gap-1.5 sm:px-3 sm:text-sm lg:mb-0 print:hidden"
    >
      {items.map((item, index) => {
        const last = index === items.length - 1;
        const showHome = index === 0 && item.path === '/dashboard';
        return (
