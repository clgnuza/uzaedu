...
import { BookOpen, Megaphone, Rss } from 'lucide-react';
...
function TvAudienceContent() {
  ...
  const slides: SlideConfig[] = useMemo(() => {
    const all: SlideConfig[] = [
      ...
      { key: 'special_day', type: 'special_day', items: specialDayItems },
      {
        key: 'principal_message',
        type: 'principal_message',
        items: byCategory.principal_message ?? [],
      },
      { key: 'staff', type: 'staff', items: byCategory.staff ?? [] },
      { key: 'birthday', type: 'birthday', items: birthdayItems },
      { key: 'success', type: 'success', items: byCategory.success ?? [] },
      { key: 'timetable', type: 'timetable', items: timetableItems },
    ...
    return all.filter((s) => s.type === 'welcome' || (s.items && s.items.length > 0));
  }, [byCategory, specialDayItems, birthdayItems, timetableItems, videoItems]);
  ...
  const visibleCardsSet = useMemo((): Set<string> | null => {
    ...
    return new Set(trimmed ? trimmed.split(',').map((s) => s.trim()).filter(Boolean) : []);
  }, [visibleCardsRaw]);
  const isCardVisible = (key: string) =>
    visibleCardsSet === null ? true : visibleCardsSet.has(key);
  ...
  const tickerItems = byCategory.ticker ?? [];
            ...
            {slides.length > 0 && (
              <div className="tv-slide-footer shrink-0 border-t border-[var(--tv-border)]/50 bg-[var(--tv-bg-dark)]/95 px-4 py-3">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="h-1 flex-1 max-w-32 overflow-hidden rounded-full bg-white/10">
...
export default function TvAudiencePage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-[#0c1929]"><LoadingSpinner label="Duyuru TV yükleniyor…" /></div>}>
      <TvAudienceContent />
    </Suspense>
  );
}
...
function SlideCard({
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      {/* Gradient – görsel yoğunluğuna göre alttan koyulaşır */}
      <div
        className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/45 to-black/10"
        aria-hidden
      />
      <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8 lg:p-10">
        <div className="tv-slide-overlay max-w-3xl py-1 md:py-2">
          ...
          {customBody ? (
            ...
            <>
              {title != null && title !== '' && (
                <h2 className={`tv-slide-title mt-3 text-white ${accentTitle ? '!text-amber-50' : ''}`}>
                  {title}
                </h2>
              )}
              {body && (
                <div className="tv-slide-body mt-3 max-h-[40vh] overflow-y-auto pr-2">
                  <p className="text-white/95 whitespace-pre-line">
                    {body}
                  </p>
                </div>
              )}
...
function SlideView({
      ...
      >
        {welcomeImageUrl && (
          <div className="absolute inset-0 overflow-hidden">
            <img src={welcomeImageUrl} alt="" className="h-full w-full object-cover opacity-20" />
            <div className="absolute inset-0 bg-gradient-to-b from-[var(--tv-bg-dark)]/70 to-[var(--tv-bg-dark)]" />
          </div>
        )}
  ...
  switch (slide.type) {
    ...
    case 'staff': {
      content = (
        <div className="relative h-full w-full overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-[var(--tv-bg-dark)] via-[var(--tv-bg)] to-[var(--tv-accent)]/10" aria-hidden />
          <div className="relative flex h-full w-full flex-col gap-6 p-6 md:p-8">
            <span className="inline-block w-fit rounded-full bg-white/20 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-white/90 backdrop-blur-sm">
              Öğretmenlerimiz
            </span>
            <div className="grid min-h-0 flex-1 grid-cols-2 auto-rows-fr gap-4 md:grid-cols-3">
              {(slide.items ?? []).slice(0, 6).map((t) => {
                const avatarUrl = (t as { attachment_url?: string }).attachment_url;
                return (
                  <div
                    key={t.id}
                    className="flex flex-col items-center justify-center rounded-xl bg-white/5 p-4 text-center md:p-5"
                  >
                    <div className="relative mb-3 h-16 w-16 overflow-hidden rounded-full bg-[var(--tv-accent)]/20 md:h-20 md:w-20">
                      {avatarUrl ? (
                    ...
                    <p className="text-base font-semibold text-[var(--tv-text)] md:text-lg">
                      {t.title || 'Öğretmen'}
                    </p>
                    <p className="mt-1 text-sm text-[var(--tv-text-muted)]">
                      {t.summary || t.body || 'Branş bilgisi'}
                    </p>
                  </div>
                );
              })}
    ...
    case 'success': {
      const imgSrc = first?.attachment_url;
      content = (
        <div className="tv-success-slide relative h-full w-full overflow-hidden">
          <div className="absolute inset-0 tv-slide-media-wrap">
            {imgSrc ? (
              <img src={imgSrc} alt="" className="tv-slide-media tv-slide-media--cover" />
            ) : (
              <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-amber-600/25 via-amber-500/15 via-35% to-[var(--tv-bg-dark)]">
                <span className="tv-success-trophy">🏆</span>
                <span className="absolute bottom-[35%] left-1/2 -translate-x-1/2 text-5xl opacity-30">✨</span>
                <span className="absolute top-[25%] right-[20%] text-4xl opacity-25">⭐</span>
                <span className="absolute top-[30%] left-[15%] text-4xl opacity-25">⭐</span>
              </div>
            )}
          </div>
          <div className="absolute inset-0 bg-gradient-to-t from-black/92 via-black/50 to-transparent" aria-hidden />
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 md:p-10 lg:p-12">
            <div className="tv-success-frame tv-slide-overlay max-w-3xl w-full px-8 py-8 md:px-12 md:py-10 text-center">
              <span className="tv-success-badge">Başarı</span>
              <h2 className="tv-success-title mt-6">
                {title || 'Tebrik ederiz!'}
              </h2>
              {body ? (
                <div className="tv-success-body tv-slide-body mt-5 max-h-[35vh] overflow-y-auto pr-2 text-center">
                  <p className="whitespace-pre-line">{body}</p>
                </div>
              ) : (
                <p className="tv-success-title mt-6 text-2xl md:text-3xl">TEBRİK EDERİZ.</p>
              )}
              <p className="mt-6 text-3xl md:text-4xl opacity-90" aria-hidden>🎉</p>
            </div>
          </div>
        </div>
      );
    ...
    case 'video': {
      ...
      content = (
        <div className="relative h-full w-full min-h-0 min-w-0 overflow-hidden bg-black">
          <div className="tv-slide-video-wrap">
            {embedUrl ? (
            <TvVideoIframe
              embedUrl={embedUrl}
            ...
            <div className="flex h-full w-full flex-col items-center justify-center gap-4 bg-[var(--tv-bg-dark)]">
              ...
              {videoItem?.title && <p className="text-sm text-white/70">{videoItem.title}</p>}
            </div>
          )}
          </div>
        </div>
