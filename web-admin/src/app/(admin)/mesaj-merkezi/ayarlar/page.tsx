...
type Provider = 'mock' | 'meta' | 'twilio' | 'netgsm' | 'custom';
type Settings = {
...
export default function AyarlarPage() {
  ...
  const [form, setForm]       = useState<Settings>({ provider: 'mock', apiKey: '', apiSecret: '', phoneNumberId: '', fromNumber: '', apiEndpoint: '', isActive: false });
  ...
  useEffect(() => {
    ...
    void (async () => {
      setLoading(true);
      try {
        const s = await apiFetch<Settings | null>(`/messaging/settings${q}`, { token });
        if (s) setForm((f) => ({ ...f, ...s }));
  ...
  const save = async () => {
    setSaving(true);
    try {
      await apiFetch(`/messaging/settings${q}`, { method: 'POST', token, body: JSON.stringify(form) });
  ...
  const test = async () => {
    ...
    try {
      ...
      if (res.ok) toast.success(`✅ Bağlantı başarılı: ${res.message}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border bg-white/80 p-5 shadow-sm dark:bg-zinc-900/60 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">Ayarları kaydettikten sonra test mesajı gönderin.</p>
